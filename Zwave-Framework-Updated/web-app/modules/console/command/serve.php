...
Class Serve{
	...
	private function run_server_login(){
		...
		$url = base_url($this->base_url."login?security_token={$security_token}&cli_action=attempt-login&username={$username}&password={$password}");
	...
	public function launch_dbadmin(){
		...
		$url = base_url($this->base_url.'phpmyadmin/?run-mode=cli&sapi=command');
		return  $this->command->run('start '.USER_AGENT.' "'.$url.'"');
	}
